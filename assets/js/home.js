...
(function () {
  ...
  const ITEMS = [
    { icon: ICON_UBICACION, title: "De dónde son", desc: "Provincia, ciudad o barrio de cada participante." },
    { icon: ICON_RELOJ, title: "Situación del distrito", desc: "Cómo ven hoy la realidad de su lugar." },
    { icon: ICON_ALERTA, title: "Problemas de la juventud", desc: "Los problemas estructurales que enfrentan en su distrito." },
    { icon: ICON_MEGAFONO, title: "Necesidades", desc: "Qué necesitan para desarrollarse." },
    { icon: ICON_VISION, title: "Visión del país", desc: "Qué tan optimistas son sobre el futuro." },
    { icon: ICON_CALENDARIO, title: "19 de septiembre", desc: "Un solo día, un solo formulario, todo el país." },
  ];

  // Grilla fija de 3 filas x 2 columnas (6 piezas) — igual que en
  // sintesis.js, la cantidad de filas/columnas no puede cambiar según el
  // ancho de pantalla o el encastre entre piezas vecinas dejaría de
  // coincidir (ver assets/js/puzzle.js).
  const ROWS = 3, COLS = 2;
  const JOINT_H = [true, false, true]; // un solo empalme horizontal por fila (2 columnas)
  const JOINT_V = [
    [false, true],
    [true, false],
  ]; // dos empalmes verticales por columna (3 filas)
  ...
  function buildGrid(el) {
    el.innerHTML = ITEMS.map((item, i) => {
      ...
      const c = i % COLS;
      const edges = window.MQD_PUZZLE.edgesFor(r, c, ROWS, COLS, (row) => JOINT_H[row], (col, k) => JOINT_V[col][k]);
